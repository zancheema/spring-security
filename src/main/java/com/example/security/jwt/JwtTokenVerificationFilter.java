...
public class JwtTokenVerificationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if (!Strings.isNullOrEmpty(authHeader) && authHeader.startsWith(jwtConfig.getPrefix())) {
            String jwtToken = authHeader.replace(jwtConfig.getPrefix(), "");
            ...
            Authentication authentication = new UsernamePasswordAuthenticationToken(
                    username, null, getGrantedAuthorities(authorities)
            );
    ...
    private Set<SimpleGrantedAuthority> getGrantedAuthorities(List<Map<String, String>> authorities) {
